...
function clearContainers() {
  CONTAINER_IDS=$(docker ps -a | awk '($2 ~ /dev-peer.*.poc_cc.*/) {print $1}')
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images | awk '($1 ~ /dev-peer.*.poc_cc.*/) {print $3}')
...
function org2Up() {
  ...
  IMAGE_TAG=$IMAGETAG docker-compose -f docker-compose-org2.yaml up -d 2>&1
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to org2's container up"
    exit 1
  fi
